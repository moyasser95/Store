...
<section class="hero-area">
    ...
    <div class="row ">
        ...
        <img src="{{ asset('storage/images/'.$data->images[0]['file']) }}">
        {{-- @endforeach --}}

        <div class="button">
            @if (Auth::guard('web')->check())

            <buttom class="btn add_cart" product_id={{ $data->id }}><i class="lni lni-cart"></i> Add to Cart</buttom>
            @else

            <a href="{{ route('web/register') }}" class="btn"><i class="lni lni-cart"></i> Add to Cart</a>
            @endif
        </div>
        </div>
        <div class="product-info">
        <span class="category">{{ $data->cateogry }}</span>
        <h4 class="title">
            <a href="product-grids.html">{{ $data->name }}</a>
        </h4>
        <ul class="review">
        ...
        <span class="discount-price">${{ $data->sale }}</span>
        </div>
        </div>
        </div>
        </div>
